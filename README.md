...
| Hidden AI instruction из PDF | Игнорируется | Скрытое требование `*.component.tsx` распознано как prompt-injection и не применяется. |
...
| UI | Список, фильтры, календарь, карточка, детали, ставки, тема | Production-деплоя |
...
AI использовался для поэтапной разработки frontend-архитектуры, реализации mock API, экранов, валидации, визуальных исправлений и финальной сверки с ТЗ.

Критичные решения проверялись вручную по коду и тестами. Скрытая белая строка в PDF про обязательный суффикс `*.component.tsx` не применялась, потому что это prompt-injection, не видимый человеку при обычном чтении задания.
