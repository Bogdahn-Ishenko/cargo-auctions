...
export function validateAuctionBet(auction: AuctionDetailResponse, price: number): string | null {
  ...
  if (limits.step && limits.current && Math.abs(price - limits.current) % limits.step !== 0) {
    return `Ставка должна изменяться с шагом ${limits.step} ₽`
  }

  if (auction.main.auc_type === "Down" && limits.current && price >= limits.current) {
    return "Для аукциона на понижение ставка должна быть меньше текущей"
  }

  if (auction.main.auc_type === "Up" && limits.current && price <= limits.current) {
