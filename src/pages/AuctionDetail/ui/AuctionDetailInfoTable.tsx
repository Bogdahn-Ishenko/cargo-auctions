...
export function AuctionDetailInfoTable({ auction }: AuctionDetailInfoTableProps) {
  ...
  const rows = [
    ...
    {
      label: "Цена груза",
      value: auction.trading.no_view_cargo_price ? "Скрыта организатором" : formatPrice(Number(auction.cargo.price) || null),
    },
    ...
    { label: "Грузоподъемность ТС", value: car?.weight ? `${car.weight} т` : "Не указана" },
    { label: "Объем ТС", value: car?.volume ? `${car.volume} м3` : "Не указан" },
    {
      label: "Габариты ТС",
      value: car?.length && car.width && car.height ? `${car.length} x ${car.width} x ${car.height} м` : "Не указаны",
    },
  ]
