...
function getActiveFilters(search: AuctionsListSearch) {
  ...
  if (search.load_date_from) filters.push(`Погрузка от ${search.load_date_from}`)
  if (search.load_date_to) filters.push(`Погрузка до ${search.load_date_to}`)
  if (search.unload_date_from) filters.push(`Выгрузка от ${search.unload_date_from}`)
  if (search.unload_date_to) filters.push(`Выгрузка до ${search.unload_date_to}`)
  if (search.weight_from) filters.push(`Вес от ${search.weight_from} т`)
  if (search.weight_to) filters.push(`Вес до ${search.weight_to} т`)
  if (search.current_price_from) filters.push(`Цена от ${search.current_price_from} ₽`)
  if (search.current_price_to) filters.push(`Цена до ${search.current_price_to} ₽`)
